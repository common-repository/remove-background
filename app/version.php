...
if ( ! function_exists( 'nobg_get_loaded_wpemerge' ) ) {
	/**
	 * Get the currently loaded WP Emerge version, if any.
	 *
	 * @return array|false
	 */
	function nobg_get_loaded_wpemerge() {
		/**
...
if ( ! function_exists( 'nobg_should_load_wpemerge' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function nobg_should_load_wpemerge( $name, $min, $max ) {
		$loaded = nobg_get_loaded_wpemerge();
		...
		add_action(
			...
			function () use ( $name, $min, $max, $loaded ) {
				...
				$message = __( '%1$s requires WP Emerge version >= %2$s and < %3$s but %4$s has already loaded version %5$s.', 'remove-background' );
				?>
...
if ( ! function_exists( 'nobg_declare_loaded_wpemerge' ) ) {
	/**
	 ...
	 * @return void
	 */
	function nobg_declare_loaded_wpemerge( $name, $type, $file ) {
		$loaded = nobg_get_loaded_wpemerge();
