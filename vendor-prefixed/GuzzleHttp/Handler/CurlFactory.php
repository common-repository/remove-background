...
use NoBg\GuzzleHttp\Exception\ConnectException;
use NoBg\GuzzleHttp\Exception\RequestException;
use NoBg\GuzzleHttp\Promise as P;
use NoBg\GuzzleHttp\Promise\FulfilledPromise;
use NoBg\GuzzleHttp\Promise\PromiseInterface;
use NoBg\GuzzleHttp\Psr7\LazyOpenStream;
use NoBg\GuzzleHttp\TransferStats;
use NoBg\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \NoBg\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \NoBg\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \NoBg\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \NoBg\GuzzleHttp\Psr7\Utils::streamFor($sink);
